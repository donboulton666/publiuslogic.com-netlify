...
export function wrapPageElement({ element }) {
  return <AnimatePresence onExitComplete={onExitComplete} mode="wait" initial={false}>{element}</AnimatePresence>
}
```
...
Using framer motion <A href="https://www.framer.com/motion/use-in-view/" target="_blank" area-label="useInView" alt="useInView">useInView</A>
